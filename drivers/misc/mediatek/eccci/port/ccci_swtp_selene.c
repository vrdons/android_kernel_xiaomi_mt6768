...
static int swtp_switch_state(int irq, struct swtp_t *swtp)
{
	...
	if (swtp->eint_type[i] == IRQ_TYPE_LEVEL_LOW) {
		...
		input_report_key(swtp_input_dev, KEY_TABLE2, 1);
		input_sync(swtp_input_dev);
		input_report_key(swtp_input_dev, KEY_TABLE2, 0);
		input_sync(swtp_input_dev);
		printk("[swtp]input keycode = %d", KEY_TABLE2);
	}
...
int swtp_init(int md_id)
{
	...
	for (i = 0; i < MAX_PIN_NUM; i++) {
		...
		input_set_capability(swtp_input_dev, EV_KEY, KEY_TABLE2);
